...
class GroundDeparturesScenario(BaseScenario):
    ...
    def generate(self, num_departures: int, spawn_delay_mode: SpawnDelayMode = SpawnDelayMode.NONE,
        ...
        # Only add warning if we couldn't generate the requested number of aircraft
        if len(self.aircraft) < num_departures:
            shortage = num_departures - len(self.aircraft)
            warning_msg = f"Generated {len(self.aircraft)}/{num_departures} departures. Missing {shortage}."
